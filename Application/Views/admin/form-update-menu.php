...
function print_tree($map, $id_parent, $shift = 0) {
    if (!empty($map)) {
        foreach ($map as $section) {
            ...
            <?php
            print_tree($section['children'], $id_parent, $shift + 2);
        }
    }
}
?>
...
<form class="admin" action="" method="post">
    ...
    <div>
        <label for="alias">Алиас: </label><input type="text" name="alias" id="alias" value="<?= $this->menu->alias; ?>" />
    </div>
    <div>
        <input type="button" id="btnCreateItemMenu" value="Добавить пункт меню"/>
    </div>
...
<?php foreach ($this->items as $item): ?>
        ...
        <div class="menu-item border">
            <div>
                <label for="title">Название пункта меню: 
                </label><input type="text" id="title_item" name="name_item[]" value="<?= $item['name_item'] ?>">
            </div>
            <div><label for="ids_parent">Категория</label><select name="ids_parent[]">
                    <option value="0">Без раздела</option>
    <?= print_tree($this->map, $item['id_category']); ?>
                </select>
                <label for="ids_page">Выберите страницу</label><select name="ids_page[]">
                    <option value="0">Все страницы</option>
                        <?php foreach ($this->pages as $page): ?>
                        <option data-parent="<?= $page->getParents() ?>" value="<?= $page->id ?>" <?php if ($page->id == $item['id_page']) echo "selected"; ?>>
                        <?= $page->title; ?>
                        </option>
    <?php endforeach; ?>
                </select><br>
...
<?php endforeach; ?>
    <div><label for="is_active">Активно: </label><input type="checkbox" name="is_active" value="1" <?php if ($this->page->is_active) {
