    ...
    function print_tree($map, $shift = 0)
    {
        if(!empty($map))
        {
            ...
            { ?>
                ...
                <?php print_tree($section['children'], $shift + 2); 
            }
        }
    }
?>
...
<form class="admin" method="POST" enctype="multipart/form-data">    
    ...
    <div>
        ...
        <label for="is_active">Отображать на главной: </label><input type="checkbox" name="is_main" value="1" <?php if ($this->page->is_main){echo 'checked="checked"';} ?> />
    </div>
